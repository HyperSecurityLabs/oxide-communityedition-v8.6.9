...
impl HybridScanner {
    ...
    pub async fn run_hybrid_scan(&mut self) -> Result<Vec<Finding>> {
        ...
        if modules.contains(&"all".to_string()) || modules.contains(&"fuzz".to_string()) {
            if !excluded.contains(&"fuzz".to_string()) {
                ...
                if !self.args.verbose {
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ...
                            let rate = if elapsed.as_secs_f64() > 0.0 { req as f64 / elapsed.as_secs_f64() } else { 0.0 };

                            let spin_bracketed = format!("{}[{}{}]{}",
                                tc("", ELITE_CYAN_B),
                                tc(cw, ELITE_CYAN_B), tc(ccw, ELITE_BLUE),
                                tc("", ELITE_CYAN_B));

                            let mod_share = mod_labels.len().max(1);
                            let blocks: String = (0..10).map(|i| {
                                let threshold = ((i + 1) as f64 / 10.0 * mod_share as f64) as usize;
                                if mod_idx.saturating_add(1) >= threshold {
                                    format!("\x1B[38;2;80;220;160m▰\x1B[0m")
                                } else {
                                    format!("\x1B[38;2;85;124;148m▱\x1B[0m")
                                }
                            }).collect();
                            let bar = blocks;
                            ...
                            let pip: String = mod_labels.iter().enumerate().map(|(i, l)| {
                                if i == mod_idx {
                                    format!("──▶ {}", format!("\x1B[38;2;80;220;160m{}\x1B[0m", l))
                                } else {
                                    format!("\x1B[38;2;100;170;255m{}\x1B[0m", l)
                                }
                            }).collect::<Vec<_>>().join("→");

                            let det_s = if det > 0 {
                                format!("\x1B[91;1m{}\x1B[0m", det)
                            } else {
                                format!("\x1B[90m{}\x1B[0m", det)
                            };
                            let err_s = if err > 0 {
                                format!("\x1B[93m{}\x1B[0m", err)
                            } else {
                                format!("\x1B[90m{}\x1B[0m", err)
                            };

                            let border = format!("\x1B[38;2;85;124;148m {}\x1B[0m", "\u{2502}");
                            ...
                            println!("\r\x1B[2K  {}  [{}]  {}  \x1B[38;2;100;210;255m{:>5.1}%\x1B[0m  \x1B[97m{:>4}/{}\x1B[0m  \x1B[91mdet:\x1B[0m{}  \x1B[90merr:\x1B[0m{}  \x1B[90m{:>5.1}s\x1B[0m{}",
                                spin_bracketed,
                                bar,
                                pip,
                                pct * 100.0,
                            ...
                            let url_display = if cur_fuzz.len() > 55 {
                                format!("…{}", &cur_fuzz[cur_fuzz.len().saturating_sub(54)..])
                            } else {
                                cur_fuzz.clone()
                            };
                            println!("\r\x1B[2K  {} {} {}",
                                tc("\u{25B6}", ELITE_BLUE),
                                tc(&url_display, ELITE_CYAN),
                                border);

                            let status = format!("{} req  \u{00B7}  req/s:{:.0}", req, rate);
                            println!("\r\x1B[2K  {} {} {}",
                                tc("\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}\u{2500}", ELITE_KALI),
                                tc(&status, ELITE_CYAN),
                                border);
                ...
                //
                //  crawled_urls.chunks(3) splits the URL list into blocks of 3,
                //  fuzz_url() tests all payload types on one URL asynchronously,
                //  join_all(futs) waits for all 3 parallel requests to complete.
                ...
                for chunk in crawled_urls.chunks(concurrency) {
                    ...
                    let futs: Vec<_> = chunk.iter().map(|url| {
                    ...
                    for (result, url) in results.into_iter().zip(chunk.iter()) {
                        match result {
    ...
    async fn crawl_phase(&mut self) -> Result<Vec<String>> {
        let result = if let Some(ref mut hc) = self.headless_crawler {
            println!("  {} {}",
                tc("◆", OSAKA_JADE_B),
                tc("Headless Chrome crawling enabled — real JS rendering", LAVENDER_BLUE));
...
use crate::cli::display::{
    ...
    ELITE_BLUE, ELITE_CYAN, ELITE_CYAN_B, ELITE_KALI,
