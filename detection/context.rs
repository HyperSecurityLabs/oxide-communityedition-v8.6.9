...
// ■ scraperクレートを使用してDOMツリーを解析し、ペイロードがどの
//   コンテキスト（HTML本文、属性、スクリプト、スタイル等）に反映されるか特定
...
impl ContextAnalyzer {
    ...
    // ♢ Classifies attribute context — URL属性か通常属性か、引用符の種類
    ...
    // ◆ 特殊文字がエンコードされずに通過していれば、その文字はフィルターを通過
    // ◆ デフォルト: 全ブロック (安全側)
    ...
    // ◆ 各 ReflectionContext に対して、必要な文字が通過している場合のみ生成
