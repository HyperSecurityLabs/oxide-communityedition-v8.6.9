// ----------------------------------------------------------------------------
//  tls_scanner.rs — TLS/SSL security scanner
// ----------------------------------------------------------------------------
//  Assesses TLS configuration by probing protocol versions (SSLv2 through
//  TLSv1.3), cipher suite strength, certificate validity (expiry, key length,
//  signature algorithm), HSTS implementation, and known vulnerabilities
//  (Heartbleed, CRIME) via real TLS handshakes and raw certificate parsing.
//
//  --- Developers ---------------------------------------------------------------
...
// ◆ TLS/SSLセキュリティ評価 / TLS/SSL security assessment techniques:
//   ① Protocol version probing: builds version-specific ClientHello (SSLv2
//      thru TLSv1.3), validates negotiated version in ServerHello response,
//      not just any handshake (eliminates false positives)
//   ② Certificate parsing: raw DER/ASN.1 parser — extracts subject CN,
//      issuer, validity dates, RSA key bits, signature algorithm OID,
//      wildcard flag, self-signed detection
//   ③ Cipher suite strength: probes with weak-only cipher list (RC4, 3DES,
//      NULL), checks Server header for outdated OpenSSL versions
//   ④ HSTS evaluation: max-age parsing, includeSubDomains, preload readiness
//   ⑤ TLS headers: Expect-CT, cookie security flags (Secure, HttpOnly, SameSite)
//   ⑥ Known vulnerability probes:
//      ★ Heartbleed (CVE-2014-0160): malformed heartbeat (length=16384,
//        payload=3 bytes), vulnerable if server responds with content_type 0x18
//      ★ CRIME: TLS compression check — offers DEFLATE(0x01), verifies
//        ServerHello compression method ≠ null
// ⚡ All probes send real TLS handshakes with proper SNI — no dummy data.
//     Certificate parser walks TBSCertificate fields: version → serialNumber →
...
impl TlsScanner {
    ...
    async fn parse_certificate(&self, host: &str, port: u16) -> Result<CertInfo> {
        ...
        // Loop-read until we have the full handshake (Certificate msg or timeout)
        let mut buf = vec![0u8; 65536];
        let mut total = 0usize;
        let read_deadline = tokio::time::Instant::now() + self.timeout;
        loop {
            if total >= buf.len() { break; }
            let remaining = tokio::time::Instant::now() < read_deadline;
            if !remaining { break; }
            let chunk = timeout(
            ...
            match chunk {
                ...
                Ok(Ok(n)) => {
                    total += n;
                    // Check if we've received a Certificate handshake message
                    if Self::has_certificate_message(&buf[..total]) {
                        break;
                    }
                    // If we have enough for a full handshake flight, stop
                    if total > 5 && buf[0] == 0x15 {
                        break; // Alert — server rejected our hello
                    }
                }
                _ => break,
            }
        ...
        if total < 5 {
            return Err(anyhow::anyhow!("Response too short ({} bytes)", total));
        }

        Self::extract_cert_info(&buf[..total], host)
    }

    /// Check if the received data contains a Certificate handshake message (0x0b).
    fn has_certificate_message(data: &[u8]) -> bool {
        let mut pos = 0;
        while pos + 5 <= data.len() {
            ...
            if payload_end > data.len() { return false; } // incomplete record
            if content_type == 0x16 {
                let mut hpos = payload_start;
                while hpos + 4 <= payload_end {
                    let hs_type = data[hpos];
                    if hs_type == 0x0b { return true; }
    ...
    fn parse_algorithm_identifier(data: &[u8]) -> String {
        ...
        let (_, len_bytes) = Self::asn1_decode_length(oid_tlv.get(1..).unwrap_or(&[])).unwrap_or((0, 0));
        if 1 + len_bytes >= oid_tlv.len() {
        ...
        while !cursor.is_empty() {
            if cursor.first() == Some(&0x03) {
                ...
                if let Some((bs_tlv, _)) = Self::asn1_read_tlv(cursor) {
                    let (_, lb) = Self::asn1_decode_length(&bs_tlv[1..]).unwrap_or((0, 0));
                            ...
                            if let Some(rsa_seq) = Self::asn1_unwrap_sequence(inner) {
                                ...
                                if let Some((mod_tlv, _)) = Self::asn1_read_tlv(rsa_seq) {
                                    if mod_tlv.first() == Some(&0x02) {
                                        let (_, lb2) = Self::asn1_decode_length(&mod_tlv[1..]).unwrap_or((0, 0));
    ...
    /// actually negotiates that version — not just any ServerHello.
    /// Previously this checked only content type + handshake type, which
    /// caused false positives: a server rejecting SSLv3 but responding
    /// with a TLS 1.2 ServerHello would be falsely flagged as "SSLv3 Enabled".
    async fn probe_tls_version(&self, host: &str, version: &str) -> bool {
        let port = 443u16;
        let addr = format!("{}:{}", host, port);

        let (record_version, expected_sh_version): ([u8; 2], [u8; 2]) = match version {
            "SSLv3"   => ([0x03, 0x00], [0x03, 0x00]),
            "TLSv1.0" => ([0x03, 0x01], [0x03, 0x01]),
            "TLSv1.1" => ([0x03, 0x02], [0x03, 0x02]),
            "TLSv1.2" => ([0x03, 0x03], [0x03, 0x03]),
            "TLSv1.3" => ([0x03, 0x03], [0x03, 0x04]), // TLS 1.3 uses supported_versions
        ...
        // Must have at least a TLS record header (5) + handshake header (4) + version (2) = 11
        if n < 11 { return false; }
        // ServerHello: content_type=0x16, handshake_type=0x02
        if buf[0] != 0x16 || buf[5] != 0x02 { return false; }
        // Verify the negotiated version in the ServerHello matches the probed version
        // ServerHello version is at offset 9-10 (record_hdr=5, hs_hdr=4)
        let sh_version = [buf[9], buf[10]];
        if sh_version == expected_sh_version { return true; }
        // For TLS 1.3, the ServerHello may use a different version but have
        // supported_versions indicating 0x0304; accept TLS 1.2 record version
        // as many servers advertise 1.3 via supported_versions extension only
        if version == "TLSv1.3" && sh_version[0] == 0x03 && (sh_version[1] == 0x03 || sh_version[1] == 0x04) {
            return true;
        }
        false
    }
    ...
    fn build_legacy_hello(host: &str, client_version: [u8; 2]) -> Vec<u8> {
        ...
        // Broad cipher list covering modern + legacy for accurate version probing
        let ciphers: &[u8] = &[
            0x00, 0x06,             // cipher list length: 6 suites (12 bytes)
            0xc0, 0x2b,             // TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256
    ...
    async fn check_cipher_suites(&self, target: &str) -> Vec<TlsFinding> {
        ...
        // Check HTTP Server header for outdated TLS stacks
        ...
        match timeout(Duration::from_secs(8), self.probe_weak_ciphers(&host)).await {
            Ok(true) => {
                findings.push(TlsFinding {
                    severity: TlsSeverity::High,
                    title: "Weak Ciphers Accepted".to_string(),
                    description: "Server accepts connections offering only weak ciphers (RC4, 3DES, or NULL).".to_string(),
                    evidence: "ClientHello with weak-only cipher list was accepted by server".to_string(),
                    remediation: "Disable RC4, 3DES, NULL, and EXPORT ciphers. Enable only AEAD ciphers (GCM/CHACHA20).".to_string(),
                });
            }
            _ => {}
        }
    ...
    /// Probe whether the server accepts connections with only weak ciphers.
    async fn probe_weak_ciphers(&self, host: &str) -> bool {
        let addr = format!("{}:443", host);
        let Ok(Ok(mut stream)) = timeout(Duration::from_secs(5), TcpStream::connect(&addr)).await
        else { return false; };
        ...
        // Only weak/outdated ciphers: RC4, 3DES, NULL
        let weak_ciphers: &[u8] = &[
            0x00, 0x08,             // length: 8 bytes (4 suites)
            0x00, 0x04,             // TLS_RSA_WITH_RC4_128_SHA
            0x00, 0x0a,             // TLS_RSA_WITH_3DES_EDE_CBC_SHA
            0x00, 0x0b,             // TLS_RSA_WITH_NULL_SHA
            0xc0, 0x08,             // TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA
        ];
        ...
        body.extend_from_slice(&[0x00u8; 32]); // random
        body.push(0x00); // session id len
        body.extend_from_slice(weak_ciphers);
        body.extend_from_slice(&[0x01, 0x00]); // compression: null
        ...
        // ServerHello (0x16, 0x02) == accepted, Alert (0x15) == rejected
        n >= 6 && buf[0] == 0x16 && buf[5] == 0x02
    }
    ...
    async fn check_hsts(&self, target: &str) -> Vec<TlsFinding> {
        ...
        if let Some(hsts) = headers.get("strict-transport-security").and_then(|v| v.to_str().ok()) {
            // Handle optional whitespace: max-age=31536000 or max-age = 31536000
            if let Some(age_start) = hsts.to_lowercase().find("max-age") {
                let after_key = &hsts[age_start + 7..];
                let after_eq = after_key.trim_start().strip_prefix('=').unwrap_or("");
                let age_str = after_eq.trim_start();
                let age_end = age_str.find(|c: char| !c.is_ascii_digit()).unwrap_or(age_str.len());
                if let Ok(age) = age_str[..age_end].parse::<u64>() {
                    if age < 2_592_000 {
                        findings.push(TlsFinding {
                            severity: TlsSeverity::Medium,
