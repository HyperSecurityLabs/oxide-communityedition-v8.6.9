// ----------------------------------------------------------------------------
//  common_app_scanner.rs — common web application scanner
// ----------------------------------------------------------------------------
//  Fingerprints web applications by probing thousands of known paths for CGI
//  scripts, admin interfaces, config files, backup files, database dumps,
//  VCS metadata, cloud credentials, and development tool endpoints — similar
//  to Nikto-style reconnaissance for attack surface discovery.
//
//  --- Developers ---------------------------------------------------------------
...
// ◆ 電脳走査手法 / scan methodology (Nikto-style):
//   Probes 6000+ known paths across categories for attack surface discovery:
//   ■ CGI scripts — CVE-based tests (Shellshock CVE-2014-6271, PHPCGI CVE-2012-1823,
//     FormMail, AWStats, Count.cgi, + 100+ modern CGI paths)
//   ■ Admin interfaces — 60+ admin panel paths (Tomcat, Jenkins, WordPress, phpMyAdmin)
//   ■ Config files — .env, config.php, wp-config.php, web.config
//   ■ Backup files — .bak, .old, .sql, .tar.gz on common filenames
//   ■ Database dumps — .sql, .sqlite, .mdb, .mdf exposures
//   ■ VCS metadata — .git, .svn, .hg directory exposure
//   ■ Cloud credentials — AWS, Azure, GCP, Kubernetes, Terraform
//   ■ Dev tools — phpinfo, debug endpoints, Swagger, GraphQL, actuators
//   ■ API endpoints — REST / v1/v2, internal/admin APIs
//   ◆ Uses precision.rs multi-layer validation to eliminate false positives
...
impl CommonAppScanner {
    ...
    fn load_cgi_tests(&mut self) {
        ...
        for (path, desc) in modern_cgi_paths {
            self.tests.push(AppTest {
                path: path.to_string(),
                method: "GET".to_string(),
                expected_status: vec![200, 403, 401, 500],
        ...
        let cgi_total = self.tests.iter().filter(|t| t.category == TestCategory::Cgi).count();
        let cve_count = 41usize;
        let modern_count = 118usize;
        let alt_count = alt_paths.len() * common_cgi_names.len() * cgi_extensions.len();
        let param_count = 30usize;
        let php_count = 14usize;
        println!("[CGI-DB] Loaded {} CGI tests [CVE:{} Modern:{} Alt:{} Param:{} PHP-CGI:{}]",
            cgi_total, cve_count, modern_count, alt_count, param_count, php_count);
    }
    ...
    fn load_admin_tests(&mut self) {
        ...
        for (path, desc) in admin_paths {
            self.tests.push(AppTest {
                ...
                method: "GET".to_string(),
                expected_status: vec![200],
                expected_content: vec!["type=\"password\"".to_string(), "type='password'".to_string(), "login-form".to_string(), "login_form".to_string()],
    ...
    fn load_config_tests(&mut self) {
        ...
        for (path, desc) in config_files {
            self.tests.push(AppTest {
                ...
                method: "GET".to_string(),
                expected_status: vec![200],
                expected_content: vec!["DB_PASSWORD".to_string(), "DB_USERNAME".to_string(), "DB_HOST".to_string(), "secret_key".to_string(), "api_key".to_string()],
                severity: Severity::Critical,
    ...
    fn load_vcs_tests(&mut self) {
        ...
        for (path, desc) in vcs_paths {
            self.tests.push(AppTest {
                ...
                method: "GET".to_string(),
                expected_status: vec![200],
                expected_content: vec!["[core]".to_string(), "ref: refs/".to_string(), "repositoryformatversion".to_string(), "fncache".to_string(), "dotencode".to_string()],
                severity: Severity::High,
    ...
    fn load_cloud_tests(&mut self) {
        ...
        for (path, desc) in cloud_paths {
            self.tests.push(AppTest {
                ...
                method: "GET".to_string(),
                expected_status: vec![200],
                expected_content: vec!["aws_secret_access_key".to_string(), "api_key".to_string(), "secret_token".to_string(), "access_key".to_string()],
                severity: Severity::Critical,
    ...
    fn load_csv_tests(&mut self) {
        ...
        println!("{BRIGHT_GREEN}[DB-LOADER]{RESET} Database found at: {BRIGHT_WHITE}{}{RESET}", enc_path.display());
    ...
    fn load_csv_tests_fallback(&mut self) {
        ...
        let csv_files = vec![
            ...
            csv_base("tests_medium.csv"),
            csv_base("tests_large.csv"),
