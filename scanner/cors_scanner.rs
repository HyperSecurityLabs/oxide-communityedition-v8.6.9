...
// ----------------------------------------------------------------------------
//  Tests for overly permissive CORS policies that could allow cross-origin
//  data theft. Checks wildcard origins with credentials, arbitrary origin
//  reflection, null origin acceptance, subdomain trust issues, HTTP-on-HTTPS
//  origins, dangerous methods, and exposed sensitive headers.
//
//  --- Developers ---------------------------------------------------------------
...
// ◆ テスト項目一覧 / CORS test categories:
//   ① Wildcard + credentials    ★ ACAO:* + ACAC:true = critical — any site reads auth data
//   ② Arbitrary origin ref.     ☆ Server echoes Origin header back without validation
//   ③ Null origin               ▲ null accepted — exploitable via sandboxed iframes
//   ④ Subdomain trust           ♢ evil.target.com accepted via overly broad policy
//   ⑤ HTTP-on-HTTPS             ◆ Mixing schemes enables MITM attacks
//   ⑥ Dangerous methods         ★ PUT/DELETE/PATCH allowed cross-origin
//   ⑦ Sensitive headers          ■ Authorization/Cookie exposed to third parties
//   ⑧ Long max-age               ➤ 24h+ cache locks in bad policy
//   ⑨ Special origins            ★ file://, data://, javascript:// accepted
//   ⑩ Preflight caching          ☆ Missing Vary: Origin causes cache poisoning
