// ----------------------------------------------------------------------------
//  mod.rs — Instagram OSINT module
// ----------------------------------------------------------------------------
//  Instagram OSINT module — scrapes public Instagram profiles for reconnaissance
//
//  --- Developers ---------------------------------------------------------------
...
// ◆ INSTAGRAM_BASE — InstagramベースURL
const INSTAGRAM_BASE: &str = "https://www.instagram.com";

// ◆ InstaProfile — Instagramプロフィール情報 / scraped profile data
// ◆ ■ username, full_name, bio: text metadata
// ◆ ■ follower_count, following_count, post_count: numeric stats
// ◆ ■ is_private, is_verified: boolean flags
// ◆ ■ profile_pic_url: URL for avatar download
// ◆ ■ tracking_id: extracted from target URL id= parameter
#[derive(Debug, Clone)]
...
impl InstaOSINT {
    ...
    // ◆ scan_profile() — Instagramプロフィールスクレイピング / profile scraping
    // ◆ ■ Extracts username from target URL (supports both @user and full URLs)
    // ◆ ■ Extracts tracking id parameter (id=xxx) from target
    // ◆ ■ Fetches profile page with browser-like headers
    // ◆ ■ Parses OG meta tags: og:title (full_name), description (bio), og:image (avatar)
    // ◆ ■ Parses JSON-embedded counts: edge_followed_by, edge_follow, edge_owner_to_timeline_media
    // ◆ ■ Falls back to text-based count extraction from HTML spans
    // ◆ ■ Detects is_private / is_verified from JSON strings in body
    ...
    // ◆ follower_count() — フォロワー数取得 / quick follower count lookup
    // ◆ ■ Delegates to scan_profile() and returns follower_count field
    // ◆ ■ Convenience method — avoids full profile struct for single-value queries
    ...
    // ◆ download_profile_pic() — プロフィール画像ダウンロード / profile pic download
    // ◆ ■ Scans profile to get profile_pic_url
    // ◆ ■ Creates output directory if missing
    // ◆ ■ Downloads image binary from Instagram CDN
    // ◆ ■ Saves as {username}_{tracking_id}.{jpg|png} in output dir
    // ◆ ■ Returns PathBuf to saved file
    ...
    // ◆ full_scan() — 総合Instagram OSINT電脳走査 / comprehensive OSINT scan
    // ◆ ■ Profile info finding (Info severity)
    // ◆ ■ Private account warning (Medium severity)
    // ◆ ■ Verified account badge (Info severity)
    // ◆ ■ Tracking parameter disclosure (Info severity)
    // ◆ ■ Returns Vec<Finding> for integration with main scan report
