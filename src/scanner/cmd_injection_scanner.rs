// ----------------------------------------------------------------------------
//  cmd_injection_scanner.rs — command injection scanner
// ----------------------------------------------------------------------------
//  Detects OS command injection vulnerabilities by injecting shell metacharacters
//  (;, |, &, `, $()) alongside Unix (id, whoami) and Windows (dir, type) commands.
//  Confirms execution through response content analysis and time-based delays.
//
//  --- Developers ---------------------------------------------------------------
...
//  電脳検出戦略 / detection strategy:
//   Injects shell metacharacters (; | & ` $()) with Unix and Windows commands.
//    Unix payloads: id, whoami, uname -a, cat /etc/passwd, ls -la, ps aux
//    Windows payloads: type win.ini, dir, hostname, systeminfo, net user
//    Time-based: sleep N / ping -c N  measure delay against baseline
//    Encoded bypass: URL/base64/hex variants to evade WAF filters
//    Flow: baseline  Unix/Windows injection  time-based  encoded  post-exploit
//   Uses multi-indicator scoring (4+ points) to eliminate false positives:
//    Primary (2pts): uid=/gid=, unix perms, shell paths in response
//    Secondary (1pt): /etc/passwd string, netstat, ifconfig references
...
impl CmdInjectionScanner {
    ...
    /// Uses structural patterns that are unlikely in normal web content.
    fn looks_like_command_output(&self, text: &str) -> bool {
        let command_patterns = [
            "uid=", "gid=",
            "drwxr-xr-x", "-rw-r--r--", "lrwxrwxrwx",
            "tcp ", "udp ",
            "LISTEN", "ESTABLISHED",
            "total ",
        ];
    ...
    /// Requires response to contain command output structure, not just generic words.
    pub async fn verify_shell_access(&self, base_url: &str, shell_path: &str) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Ok(response) = self.client.get(&shell_url).await {
            ...
            // Must have command output structure — username on a line without HTML
            Ok(lower.lines().any(|l| {
                let t = l.trim();
                !t.contains('<') && !t.contains('>') && t.len() <= 64 && !t.is_empty() &&
                (t.starts_with("uid=") || (!t.contains(' ') && t.len() <= 32))
            }))
        } else {
            Ok(false)
        }
        ...
        for payload in payloads.iter().take(20) {
            ...
            match response {
                Ok(resp) => {
                    ...
                    // Use scorer to check meaningful difference from baseline
                    let diff_score = crate::detection::scorer::Scorer::response_diff_score(&baseline_response, &response_text);
                    let ngram_sim = crate::detection::scorer::Scorer::ngram_cosine(&baseline_response, &response_text, 3);
                    let bayes = crate::detection::scorer::Scorer::bayesian_confidence(&[diff_score, 1.0 - ngram_sim], 0.1);
                    if !crate::detection::scorer::Scorer::passes_threshold(bayes, 0.55) {
                        continue;
                    }
    ...
    /// Uses strict multi-indicator scoring to eliminate false positives.
    fn contains_unix_command_output(&self, response: &str, payload: &str) -> bool {
        ...
        // Secondary indicators - need multiple to confirm
        ...
        // Need at least 4 points to trigger
        ...
        // whoami: require the username on a line by itself (no HTML wrapping, no spaces, short)
        if payload.contains("whoami") && 
           lower_response.lines().any(|line| {
               let t = line.trim();
               !t.contains('<') && !t.contains('>') && !t.is_empty() && t.len() <= 32 && !t.contains(' ')
           }) {
            return true;
        }
        
        // uname: require both "linux" and "kernel" AND a version number
        if payload.contains("uname") && 
           lower_response.contains("linux") && lower_response.contains("kernel") &&
           lower_response.chars().filter(|&c| c == '.').count() >= 2 {
            return true;
        }
    ...
    //  時間ベース電脳検出 / time-based command detection:
    //   Establishes baseline (avg of 2 requests), injects sleep/ping payloads,
    //   compares elapsed time against threshold = baseline + 95% of expected delay.
    //    Response body mentioning "sleep" is NOT evidence — only actual delay counts
    //    Guards against false positives from WAF delays or network latency
    ...
    async fn test_time_based_commands(&self, url: &str, param: &str) -> Option<Finding> {
        ...
        for (payload, expected_secs) in &time_payloads {
            ...
            let threshold_ms = baseline_ms + (expected_secs * 1000 * 95 / 100); // 95% of expected delay
