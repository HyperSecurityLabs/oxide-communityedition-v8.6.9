// ----------------------------------------------------------------------------
//  blind_sqli_scanner.rs — blind SQL injection scanner
// ----------------------------------------------------------------------------
//  Detects blind SQL injection vulnerabilities using time-based delays
//  (SLEEP, WAITFOR, pg_sleep) and boolean-based comparisons that produce
//  distinct response patterns between true/false conditions — enabling
//  data extraction without direct output reflection.
//
//  --- Developers ---------------------------------------------------------------
...
//   Two complementary techniques for blind SQL injection — no error messages
//   are visible, so detection relies on side-channel inference:
//    Boolean-based: inject true/false conditions (AND '1'='1 vs '1'='2),
//     compare response content/length differences. Threshold: 10% length diff
//    Time-based: inject SLEEP/WAITFOR/pg_sleep, measure delay against
//     established baseline. Uses 3 trials per payload, 2.5x threshold multiplier
//   Flow: baseline  boolean tests  time tests  DB fingerprint extraction
...
impl BlindSqliScanner {
    ...
    async fn establish_baseline(&mut self, url: &str, params: &[String]) -> Result<()> {
        ...
        if times.is_empty() {
            self.baseline_time = Duration::from_millis(500);
            return Ok(());
        }
    ...
    //  ブーリアンベース / boolean-based detection:
    //   Sends paired payloads ('1'='1 vs '1'='2), detects vulnerability when
    //   responses differ in content (has_different_content) or length
    //   (has_different_length) beyond the 10% threshold.
    //    True condition  baseline-like response
    //    False condition  notably different response OR SQL error
    ...
    //  タイムベース / time-based detection:
    //   Injects SLEEP(n)/WAITFOR/pg_sleep payloads with expected delays.
    //    Each payload tested 3 times  average actual delay
    //    If avg_delay > baseline  2.5 AND avg_delay  expected  0.8  vulnerable
    //   Uses tokio::timeout(10s) to prevent hanging on patched servers
    ...
    async fn test_time_blind_sqli(&mut self, url: &str, param: &str) -> Option<Finding> {
        ...
        for (payload, expected_delay) in time_payloads {
            ...
            if !delays.is_empty() {
                let avg_delay = delays.iter().sum::<Duration>() / delays.len().max(1) as u32;
    ...
    fn has_different_content(&self, resp1: &str, resp2: &str) -> bool {
        // Check for SQL-specific error/warning patterns to avoid false positives
        let sql_error_patterns = ["SQL syntax", "mysql_fetch", "ORA-", "syntax error", "MySQL error", "PostgreSQL error", "SQLite error"];
        let sql_warning_patterns = ["Warning: mysql_", "PHP Warning: mysql"];
        resp1.len() != resp2.len() || 
        sql_error_patterns.iter().any(|p| resp1.contains(p)) != sql_error_patterns.iter().any(|p| resp2.contains(p)) ||
        sql_warning_patterns.iter().any(|p| resp1.contains(p)) != sql_warning_patterns.iter().any(|p| resp2.contains(p))
    }
