// ----------------------------------------------------------------------------
//  rate_limiter.rs — adaptive rate limiter
// ----------------------------------------------------------------------------
//  adaptive rate limiter — token-bucket algorithm, adjusts to server response patterns
//
//  --- Developers ---------------------------------------------------------------
...
//  AdaptiveRateLimiter — token-bucket + circuit breaker / 適応型レートリミッター
//  Architecture / アーキテクチャ:
//    config — base_rps, max_rps, min_rps, window_size, circuit_threshold, etc.
//    request_history — sliding window of RequestRecord (timestamp, success, time)
//    circuit_state — Closed | Open | Half-Open state machine
//    consecutive_failures — counter for circuit breaker threshold
//  Rate limiting flow / レート制限フロー:
//   1. Allow check: circuit breaker state  if Open, reject (unless timed out)
//   2. Calculate current RPS from 1-second sliding window
//   3. Calculate target RPS based on recent success rate + avg response time
//   4. Allow if current < target (token-bucket semantics)
//  Adaptive adjustment / 適応調整:
//    success_rate > 95% + avg_time < 500ms  increase rate (1.5, capped at max_rps)
//    success_rate < 80% or avg_time > 2000ms  decrease rate (1.5, floored at min_rps)
//  Circuit breaker: 5 consecutive failures  Open for 30s  Half-Open  Closed on success
//  Cleanup: stale records purged from sliding window outside config.window_size
...
impl AdaptiveRateLimiter {
    ...
    //  Allow Check / リクエスト許可判定
    //  Circuit breaker state machine:
    //    Open      reject all (check timeout  if expired, transition to Half-Open)
    //    Half-Open  allow one request (test if target recovered)
    //    Closed    normal rate limiting based on token-bucket comparison
    //  Token check: current_rps < target_rps
    //  The Half-Open state prevents thundering herd on recovery.
    ...
    //  Success Recording / 成功記録
    //  Adds a positive RequestRecord to history
    //  Resets consecutive_failures counter
    //  If circuit was Half-Open  transition to Closed (target recovered)
    //  Triggers cleanup of old records
    //  Accurate success/failure recording is critical for adaptive rate adjustment.
    ...
    //  Failure Recording / 失敗記録
    //  Adds a negative RequestRecord to history
    //  Increments consecutive_failures
    //  If consecutive_failures  circuit_threshold (default 5):
    //    Open circuit (reject all requests for circuit_timeout)
    //  Records last_failure timestamp for Half-Open transition timing
    //  Opening the circuit prevents wasting resources on a down/unreachable target.
    ...
    //  Adaptive Target RPS Calculation / 適応的目標RPS計算
    //  Based on two metrics from the sliding window:
    //    success_rate — proportion of successful requests
    //    avg_response_time — average response time of successful requests
    //  Decision logic / 判定ロジック:
    //    success_rate > 95%  AND avg_time < 500ms   PERFORMANCE GOOD: increase rate
    //    success_rate < 80%  OR  avg_time > 2000ms   PERFORMANCE POOR: decrease rate
    //    otherwise  maintain current rate
    //  Uses adaptive_multiplier (default 1.5) for exponential adjustments
    //  This closed feedback loop automatically finds the optimal request rate.
...
//  TokenBucket — Burst Control / トークンバケット電脳制御
//  Classic token-bucket algorithm:
//    max_tokens  — burst capacity (peak concurrency)
//    refill_rate — tokens per second sustained rate
//    tokens      — current available tokens (thread-safe via Mutex)
//    last_refill — timestamp of last refill for elapsed calculation
//  consume()  refill based on elapsed time  deduct tokens  return success/fail
//  available()  refill first  return current token count
//  Useful for short bursts while maintaining average rate limits.
