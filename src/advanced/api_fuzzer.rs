// ----------------------------------------------------------------------------
//  api_fuzzer.rs — REST API fuzzer
// ----------------------------------------------------------------------------
//  REST API fuzzer — sends crafted payloads to API endpoints, detects misconfigurations
//
//  --- Developers ---------------------------------------------------------------
...
//
//  ApiFuzzer — REST & GraphQL fuzzer / RESTおよびGraphQLファズ
//  Flow overview / フロー概要:
//   1. Load payload templates (SQLi, NoSQL, GraphQL, CMDi, XXE, SSTI)
//   2. Generate test cases for each endpoint  method  content-type  auth
//   3. Inject payloads as JSON body or URL parameters
//   4. Analyze responses to detect vulnerabilities and identify tech stack
//  The fuzzer enumerates attack surface by combining HTTP verbs, content
//   types, and payload templates into a Cartesian product of test cases.
...
impl ApiFuzzer {
    ...
    //  Payload Template Loading / ペイロードテンプレート読み込み
    //  Templates organized by vulnerability class:
    //    SQLi     — boolean/time-based blind, UNION, error-based
    //    NoSQL    — MongoDB operators ($ne, $gt, $regex, $where)
    //    GraphQL  — introspection queries, schema extraction, mutations
    //    CMDi     — command injection via ; | ` $()  chaining
    //    XXE      — XML external entity injection (file read, SSRF)
    //    SSTI     — template injection (Jinja2, Freemarker, ERB)
    //  Each template vector provides multiple variants for depth.
    ...
    //  REST Test Generation / RESTテスト生成フロー
    //  For each endpoint, generates a Cartesian product:
    //    Every HTTP method (GET/POST/PUT/DELETE/PATCH/OPTIONS/HEAD)
    //    Every content type (JSON, form, XML, GraphQL, plain text)
    //    Authentication variations (none, bearer, basic, API key)
    //    Injection payloads (body JSON + URL parameter variants)
    //  Each test case carries a `test_type` tag for later analysis.
    ...
    //  Injection Test Generation / インジェクションテスト生成
    //  Wraps each payload in two delivery mechanisms:
    //    JSON body injection   POST with Content-Type: application/json
    //    URL parameter injection  GET with ?test=<urlencoded payload>
    //  Only top-3 payloads per category used to keep test set manageable.
    ...
    //  GraphQL Test Generation / GraphQLテスト生成
    //  Tests three attack vectors:
    //    Introspection queries  extract full schema (types, fields, queries)
    //    Batch query attacks    potential DoS via multi-query batching
    //    Mutations              test for dangerous write operations
    //  Introspection is the first step — it reveals the entire API surface.
    ...
    //  Auth Bypass Testing / 認証バイパステスト
    //  Manipulates HTTP headers to bypass access controls:
    //    Header injection  X-Original-URL, X-Forwarded-For, X-Real-IP, etc.
    //    Method override  X-HTTP-Method-Override, _method parameter
    //  These exploit reverse proxy misconfigurations and path normalization.
    ...
    //  API Technology Detection / API技術電脳検出
    //  Response analysis pipeline:
    //    Parse response headers (X-Powered-By, Server, X-AspNet-Version, etc.)
    //    Check body for framework signatures (Swagger, GraphQL, debug output)
    //    Build a technology fingerprint for the target
    //  Tech fingerprint guides subsequent exploit selection.
    ...
    pub fn detect_api_tech(&self, headers: &HashMap<String, String>, body: &str) -> Vec<String> {
        ...
        for (key, value) in headers {
            ...
            if val_lower.contains("django/") || val_lower.contains("django ") { tech.push("Django".to_string()); }
            if val_lower.contains("rails/") || val_lower.contains("rails ") { tech.push("Rails".to_string()); }
        ...
        if (body.contains("Error:") && body.contains("at ")) || (body.contains("Exception") && body.contains("Stack trace:")) {
