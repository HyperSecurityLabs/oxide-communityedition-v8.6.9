...
//  Chromiumをサブプロセスとして実行しJSレンダリング後のDOMを解析
...
async fn fetch_via_chrome(url: &str, user_agent: &Option<String>, cookie: &Option<String>) -> Result<String> {
    ...
    // URL sanitization: only allow http/https, block dangerous schemes
    let sanitized = sanitize_chrome_url(url)?;
...
/// Sanitize URL for Chrome subprocess — only http/https allowed, no shell injection.
fn sanitize_chrome_url(raw: &str) -> Result<String> {
    ...
    if lower.starts_with("file:") || lower.starts_with("javascript:")
        || lower.starts_with("data:") || lower.starts_with("about:")
        || lower.starts_with("blob:") || lower.starts_with("chrome:")
        || lower.starts_with("chrome-extension:")
    {
        return Err(anyhow::anyhow!("Blocked dangerous URL scheme: {}", trimmed));
    }
    ...
    if scheme != "http" && scheme != "https" {
        return Err(anyhow::anyhow!("Only http/https URLs allowed, got scheme '{}'", scheme));
    }
    Ok(parsed.to_string())
}
