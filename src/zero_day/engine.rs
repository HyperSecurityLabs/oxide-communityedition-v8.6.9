// ----------------------------------------------------------------------------
//  engine.rs — zero-day detection engine
// ----------------------------------------------------------------------------
//  zero-day detection engine — orchestrates baseline learning, feature extraction, classification
//
//  --- Developers ---------------------------------------------------------------
...
//  ZeroDayEngine — ゼロデイ電脳検出エンジン / zero-day engine orchestrator
//   Orchestration: baseline  feature extraction  classification
//   Manages AnomalyEngine for detection, ScanStats for telemetry
//   EngineConfig: learning_mode, detection_threshold, time_based_detection
...
impl ZeroDayEngine {
    ...
    //  analyze_response() — レスポンス分析パイプライン / full analysis pipeline
    //   Step 1: Feature extraction (ResponseFeatures::from_response)
    //   Step 2: Learning mode — update baselines, check health every 100 responses,
    //    export every 500 responses for backup
    //   Step 3: Anomaly detection (AnomalyEngine::analyze)
    //   Step 4: Statistics tracking (responses_analyzed, anomalies, vulns)
    //   Step 5: Recommendation generation, severity logging
    //   Returns DetectionReport with is_zero_day classification
    ...
    //  is_zero_day() — ゼロデイ判定 / zero-day classification criteria
    //   anomaly_indicator: anomaly_score > threshold (1.5 threshold if timing disabled)
    //   vuln_indicator: vulnerability_score > detection_threshold
    //   novel_indicator: vulnerability type contains "Unknown" or "Potential"
    //   Returns true if: (anomaly AND vuln) OR (vuln AND novel)
