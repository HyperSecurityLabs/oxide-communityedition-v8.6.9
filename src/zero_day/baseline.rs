// ----------------------------------------------------------------------------
//  baseline.rs — baseline learning module
// ----------------------------------------------------------------------------
//  baseline learning module — builds statistical profiles of normal server behavior
//
//  --- Developers ---------------------------------------------------------------
...
//
//
//  EndpointBaseline — エンドポイントベースライン / per-endpoint statistical profile
//   baseline learning: builds statistical profiles of normal server responses
//    using Welford's online algorithm for numerical stability.
//   Tracks: response_time, body_length, entropy statistics per endpoint.
//   Min/max tracking for range awareness + percentile_95 estimation.
//   status_code_distribution: histogram of normal status codes.
//   content hashing: fingerprints normal response content for change detection.
//   Adaptive thresholds: mean + 3σ for response time, CV-based size threshold.
...
//  エンドポイントベースライン / per-endpoint statistical profile
//   Welford's algorithm: numerically stable single-pass mean + variance
//   FeatureStats: response_time, body_length, entropy
//   Min/max tracking, percentile_95 estimation, status code histogram
//   Content fingerprinting via hash frequency
//   Adaptive thresholds: mean + 3σ (99.7% coverage under normal dist)
...
impl EndpointBaseline {
    ...
    //  update() — ベースライン更新 / update baseline with new sample
    //   Increments sample_count, updates status_code distribution
    //   Content fingerprinting: adds hash to normal_hashes after 2 sightings
    //   Welford online statistics: O(1) per-sample mean/variance update
    //   Adaptive threshold recalculation after each update
    ...
    //  Welford's online algorithm — オンライン統計 / single-pass mean + variance
    //   For each new value xᵢ:
    //    δ = xᵢ - meanₙ₁
    //    meanₙ = meanₙ₁ + δ / n
    //    M2ₙ = M2ₙ₁ + δ  (xᵢ - meanₙ)
    //   Variance = M2 / (n-1), StdDev = variance
    //  Benefits: O(1) update, numerically stable, no raw data storage needed
    /// Update statistics using Welford's online algorithm
    ...
    //  update_thresholds() — 適応的閾値更新 / adaptive threshold learning
    //   Response time: mean + 3σ (99.7% normal range), min 100ms
    //   Body size threshold: CV-based (0.3 + coefficient_of_variation), capped at 200%
    //   Entropy threshold: 2σ observed variance, min 1.0
    //   Requires  5 samples for statistical significance
    ...
    //  is_anomaly() — 異常電脳検出 / anomaly detection against baseline
    //   Response time anomaly: z-score based, threshold > 0.7
    //   Body size anomaly: z-score based, threshold > 0.7
    //   Entropy anomaly: z-score based, threshold > 0.7
    //   Content hash anomaly: unseen content detected (requires  5 samples)
    //   Status code anomaly: deviation from most common code (requires  10 samples)
    //   Overall score: max of individual anomaly scores (not average)
    //    — prevents signal dilution from normal features
    ...
    pub fn is_anomaly(&self, features: &ResponseFeatures) -> AnomalyCheck {
        ...
        // Require at least 5 samples to establish what "normal" content looks like.
        if self.sample_count >= 5 && !self.normal_hashes.is_empty() {
        ...
        // Check status code anomaly — requires 10+ samples for a stable baseline.
        // < 10 samples is too few to know what status codes are "normal" for the endpoint.
...
//  BaselineLearner — ベースライン学習管理 / manages per-endpoint baselines
//   learn(): updates baseline + stores response history (max 50 per URL)
//   check_anomaly(): compares response against learned profile
//   normalize_url(): strips query strings, replaces numeric/UUID segments with {id}/{uuid}
//   export/import: JSON serialization for persistence (save/load)
...
impl BaselineLearner {
    ...
    //  normalize_url() — URL正規化 / pattern-based URL normalization
    //  Converts /api/users/123  /api/users/{id}
    //  Converts /api/session/uuid  /api/session/{uuid}
    //  Strips query strings and fragment IDs for endpoint matching
