...
// ----------------------------------------------------------------------------
//  anomaly detection engine — scores HTTP responses against learned baselines
//
//  --- Developers ---------------------------------------------------------------
...
//  AnomalyResult — 異常電脳検出結果
//  Combines baseline deviation, ML classification, and rule-based signals
//  into a unified anomaly score with severity and vulnerability type.
...
//  AnomalyEngine — 異常検知エンジン
//   anomaly scoring: scores each HTTP response against learned baseline
//   baseline_learner: stores per-endpoint statistical profiles
//   thresholds: configurable sensitivity for anomaly vs vulnerability
//   use_rule_based_fallback: fallback when ML model is untrained
...
impl AnomalyEngine {
    ...
    //  analyze() — 5段階パイプライン
    //  Step 1: baseline check — ベースラインとの比較 / deviation from normal
    //  Step 2: ML classification — ランダムフォレスト + SVM予測
    //  Step 3: Rule-based detection — ヒューリスティックフォールバック
    //  Step 4: Combine scores — 重み付きスコア融合
    //  Step 5: Build result — 最終判定とレポート生成
    ...
    //  classify() — ML分類 / ML classification
    //  Uses trained VulnClassifier for prediction; falls back to
    //  rule-based heuristics if model is not yet trained.
    /// ML classification
    ...
    //  combine_results() — スコア融合 / weighted score fusion
    //   ML weight: 0.5 when trained (conf > 0), else 0.1
    //   Baseline weight: 0.3 when confident (> 0.5), else 0.15
    //   Rule weight: fixed at 0.2 (lightweight heuristic)
    //  Signals are normalized and combined: vuln_score  0.7 + anomaly  0.3
    ...
    fn combine_results(
        ...
        // When baseline shows no anomaly, it contributes nothing — no phantom noise.
        let baseline_signal = if baseline.is_anomaly { baseline.score } else { 0.0 };
    ...
    //  build_result() — 最終結果構築 / build final AnomalyResult
    //  Collects reasons from all 3 signals, selects vulnerability type
    //  by highest confidence (ML > rule), calculates confidence from
    //  signal agreement, applies thresholds for final boolean flags.
    ...
    //  calculate_confidence() — 信頼度計算 / confidence from signal agreement
    //  When ML is trained: ML(60%) + baseline(30%) + rule(10%)
    //  Otherwise: baseline(50%) + rule(50%)
    //  Boost 1.2 if signals agree, penalize 0.8 if they disagree
    /// Calculate confidence based on signal agreement
    ...
    //  learn() — ベースライン学習 / learn from normal responses
    //  Updates the per-endpoint baseline with each new response.
    //  Monitors baseline age — warns if > 7 days stale.
    ...
    //  train_classifier() — ML分類器の訓練 / train ML classifier
    //  Takes labeled (features, is_vulnerable) pairs, feeds to
    //  VulnerabilityClassifier::train() for model fitting.
