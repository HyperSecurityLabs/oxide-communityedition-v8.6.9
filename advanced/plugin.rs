...
impl PluginManager {
    ...
    //   2. dlopen via libloading::Library (unsafe FFI call)
    //   3. Lookup `create_plugin` function symbol in the library
    //   4. Call create_plugin() → returns *mut dyn VulnPlugin
    //   5. Convert raw pointer to Box<dyn VulnPlugin> (take ownership)
    //   6. Register the plugin
    //   7. Store Arc<Library> to keep library loaded (prevent dangling fn pointers)
