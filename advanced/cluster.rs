// ----------------------------------------------------------------------------
//  cluster.rs — distributed scan clustering
// ----------------------------------------------------------------------------
//  distributed scan clustering — coordinates multiple scanner instances across targets
//
//  --- Developers ---------------------------------------------------------------
...
//  Cluster Master — distributed scan coordinator / 分散電脳走査調整役
//  Architecture: Master-Agent model
//    Master  : listens for agent connections, distributes tasks, collects results
//    Agent   : connects to master, executes scan tasks, reports back
//  Communication: TCP with bincode-serialized ClusterMessage enum
//    Heartbeat     — agent announces liveness + load stats
//    TaskAssign    — master assigns a target + scan type to an agent
//    TaskComplete  — agent returns findings to master
//    TaskFailed    — agent reports failure with error string
//    StatusRequest/Response — load balancing information
//  Message flow / メッセージフロー:
//   Master listens on TCP  agent connects  heartbeat loop established 
//   master assigns tasks  agent scans  results flow back to master 
//   final collection via get_all_findings()
...
impl ClusterMaster {
    ...
    //  Master Start / マスター起動フロー
    //  Listens on TCP for incoming agent connections
    //   Each connection gets a spawned task  handle_agent_connection
    //   Runs in an infinite loop — terminates on drop
    //  The master is a single point of coordination; agents are ephemeral.
    ...
    //  Agent Connection Handler / エージェント接続ハンドラ
    //  Reads serialized ClusterMessage from TCP stream (bincode)
    //    Heartbeat  register/update node status
    //    TaskComplete  collect findings into shared results vec
    //    TaskFailed  log error
    //    StatusResponse  update node load metrics
    //    On EOF (0 bytes)  connection closed, exit loop
    //  Each agent gets its own task; master does not block on any one agent.
    ...
    async fn handle_agent_connection(
        ...
        loop {
            match stream.read(&mut buffer).await {
                ...
                Ok(n) => {
                    let message: ClusterMessage = match bincode::options()
                        .with_limit(1024 * 1024) // 1 MB max message size
    ...
    //  Task Distribution / タスク分散アルゴリズム
    //  Distributes targets across healthy agents using round-robin chunks:
    //    Count healthy agents
    //    Divide targets into equal chunks per agent (last agent gets remainder)
    //    Assign each chunk to an agent with sequential task IDs
    //    Track assignments in task_queue for accountability
    //  If no healthy agents  return error early.
...
//  Cluster Agent — remote scan executor / リモート電脳走査実行者
//  Lifecycle / ライフサイクル:
//    Connect to master TCP endpoint
//    Send initial heartbeat (registers with master)
//    Spawn periodic heartbeat task (every 30s) with active task count
//    Execute assigned scans based on ScanType (Quick/Full/Stealth/Custom)
//    Return findings to master on completion
//  Agents are designed to be ephemeral and stateless — all state lives on master.
...
impl ClusterAgent {
    ...
    //  Agent Start / エージェント起動
    //  TCP connect to master address
    //  Send initial heartbeat to register
    //  Spawn background heartbeat task (30s interval, prints active task count)
    //  The main task then waits for incoming assignments
    //  Heartbeat doubles as liveness probe and load reporting.
    ...
    //  Task Execution / タスク実行ディスパッチャ
    //  Dispatches to scan type-specific methods:
    //    Quick    basic reachability + security header check
    //    Full     Quick + common sensitive paths (robots.txt, .git, /admin, /api)
    //    Stealth  slow scan with randomized User-Agent + delay
    //    Custom   user-specified checks (headers, paths, etc.)
    //  Each method uses reqwest for HTTP requests and produces Findings.
    //  Active task count tracked for heartbeat reporting.
