// ----------------------------------------------------------------------------
//  ml_detector.rs — ML-based anomaly detection
// ----------------------------------------------------------------------------
//  ML-based anomaly detection — uses trained models to classify suspicious responses
//
//  --- Developers ---------------------------------------------------------------
...
// ◆ MLDetector — ML-based Anomaly Detection / ML異常検知
// ■ Inference pipeline / 推論パイプライン:
//   1. Feature Extraction — from raw HTTP response:
//      ★ response_time_ms  — latency, useful for blind SQLi detection
//      ★ body_length       — unusual length may indicate error dumps
//      ★ header_count      — unusual header count may indicate misconfig
//      ★ error_keyword_count — presence of error-related keywords
//      ★ entropy (Shannon) — randomness metric for obfuscated content
//      ★ digit_ratio       — numeric content proportion
//      ★ uppercase_ratio   — SHOUTING content detection
//      ★ special_char_ratio— SQL/NoSQL syntax character density
//   2. Z-score normalization (subtract mean, divide by std)
//   3. Euclidean distance to normal pattern cluster
//   4. Threshold comparison → AnomalyResult (is_anomaly, confidence, type)
// ■ Two classification paths:
//   ★ statistical anomaly detection (z-score + distance)
//   ★ heuristic classification (weighted score: error, entropy, length, time)
// ■ Online learning: add_normal_pattern() extends the model incrementally.
// ■ NeuralLayer: single perceptron with sigmoid + gradient descent for advanced use.
...
impl MLDetector {
    ...
    // ■ Converts raw HTTP response into 8-dimensional feature vector:
    //   ★ Statistical: response_time, body_length, header_count
    //   ★ Lexical: error_keyword_count (error, exception, fail, invalid...)
    //   ★ Information-theoretic: Shannon entropy of body
    //   ★ Compositional: digit_ratio, uppercase_ratio, special_char_ratio
    // ➤ This vector is the input to both anomaly detection and classification.
    ...
    // ■ Measures randomness of body content:
    //   H(X) = -Σ p(x) × log₂(p(x))
    //   High entropy → likely obfuscated/encoded/compressed content
    //   Low entropy  → predictable patterns (e.g., templated errors)
    // ➤ Useful for detecting encoded payload reflections.
    ...
    // ■ Stores feature vectors of normal responses as baseline:
    //   ★ Clears existing patterns
    //   ★ Converts each ResponseFeatures → Vec<f64> feature vector
    //   ★ Labels all as ResponseType::Normal with confidence 1.0
    //   ★ Prints training summary
    // ➤ More normal samples → better statistical baseline → fewer false positives.
    ...
    // ■ Algorithm / アルゴリズム:
    //   1. Convert incoming features to vector
    //   2. Calculate mean + std of normal pattern set (z-score params)
    //   3. Z-score normalize the new feature vector
    //   4. Compute Euclidean distance to EVERY normal pattern
    //   5. Take average distance → anomaly_score = avg / threshold
    //   6. If anomaly_score > 1.0 → classify as Anomaly
    //   7. Calculate confidence based on distance margin
    // ■ Also classifies errors (error_keyword_count > 0) vs normal vs anomaly
...
// ◆ NeuralLayer — Single Perceptron / 単一パーセプトロン
// ■ Forward pass: y = σ(w·x + b) where σ is sigmoid
// ■ Training: gradient descent on binary cross-entropy loss
//   weight_update = lr × error × input × σ' (sigmoid derivative)
// ■ Used for online learning of simple classification boundaries
// ➤ A single layer has limited capacity — stack multiple for deep learning.
