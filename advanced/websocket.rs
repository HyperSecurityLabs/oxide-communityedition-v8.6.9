// ----------------------------------------------------------------------------
//  websocket.rs — WebSocket fuzzer
// ----------------------------------------------------------------------------
//  WebSocket fuzzer — tests WebSocket endpoints for injection and misconfiguration
//
//  --- Developers ---------------------------------------------------------------
...
// ◆ WebSocketTester — WebSocket fuzzer / WebSocketファザー
// ■ Vulnerability testing flow / 脆弱性テストフロー:
//   1. URL parsing (ws:// or wss://, host, port, path extraction)
//   2. TCP connection establishment (with TLS support stub)
//   3. WebSocket handshake (Sec-WebSocket-Key generation, upgrade request)
//   4. Frame injection — send fuzz payloads as masked text frames
//   5. Response analysis — SQL error, XSS reflection, command output, path traversal
//   6. Connection teardown (close frame)
// ■ Fuzz payload categories / ファズペイロードカテゴリ:
//   ★ SQL injection in WS messages
//   ★ XSS through WebSocket
//   ★ Command injection via WS
//   ★ Path traversal in file download actions
//   ★ JSON prototype pollution
//   ★ Large payloads (DoS)
//   ★ Malformed JSON
//   ★ Special character injection
//   ★ Format string attacks
// ■ Vulnerability-specific tests / 脆弱性別テスト:
//   ★ Auth bypass — connect without auth, try bypass headers
//   ★ Unauthenticated operations — test status, list, ping, subscribe
//   ★ Format confusion — binary/text opcode mixing, oversized control frames,
//     fragmented messages with mixed opcodes
// ➤ WebSockets are often overlooked — this module finds injection points in
...
impl WebSocketTester {
    ...
    // ◆ Endpoint Test / エンドポイントテストフロー
    // ■ Full vulnerability scan sequence:
    //   ★ Parse WS URL → (host, port, path, secure)
    //   ★ TCP connect to host:port
    //   ★ Perform WebSocket upgrade handshake
    //   ★ For each fuzz payload:
    //     - Build masked text frame (opcode 0x81)
    //     - Send → read response → analyze for vulnerability signatures
    //     - Brief 100ms delay between tests for stability
    //   ★ Close connection gracefully
    //   ★ Return all findings from the scan
    // ➤ Each test_endpoint call is self-contained and stateless.
    ...
    fn parse_ws_url(&self, url: &str) -> (String, u16, String, bool) {
        ...
        let (host, port) = if host_port.contains(':') {
            ...
            let port = hp.get(1).and_then(|p| p.parse().ok()).unwrap_or(if secure { 443 } else { 80 });
            (hp[0].to_string(), port)
        } else {
    ...
    // ◆ WebSocket Handshake / WebSocketハンドシェイク
    // ■ Sends RFC 6455 upgrade request:
    //   GET /path HTTP/1.1
    //   Host: host
    //   Upgrade: websocket
    //   Connection: Upgrade
    ...
    // ■ Expects 101 Switching Protocols response
    // ■ Key is generated from SystemTime nanos to ensure uniqueness
    // ➤ Handshake failure indicates the endpoint does not support WebSocket.
    ...
    // ◆ Frame Construction / WebSocketフレーム構築
    // ■ Builds a masked text frame per RFC 6455 Section 5.2:
    //   ★ Byte 0: FIN=1 (final fragment) | opcode=1 (text frame)
    //   ★ Byte 1+: payload length (7, 16, or 64 bits depending on size)
    //   ★ Mask key: 4 bytes (client MUST mask, server MUST unmask)
    //   ★ Masked payload: payload XOR mask_key (repeated every 4 bytes)
    // ■ Reads response frame and handles unmasking:
    //   ★ If masked bit set → XOR with mask key to decode
    //   ★ Returns decoded text
    // ➤ Proper frame construction is required — many naive fuzzers skip masking.
    ...
    // ◆ Response Analysis / レスポンス分析
    // ■ Scans response for 5 vulnerability indicators:
    //   ★ SQL errors → "sql syntax", "mysql error", "ora-" etc.
    //   ★ XSS reflection → "<script" or "alert(" in response
    //   ★ Command output → "uid=" AND "gid=" (UNIX id output)
    //   ★ File content → "[extensions]" or "for 16-bit app support"
    //   ★ Prototype pollution → "isadmin" or "is_admin" in response
    // ➤ Returns the first matching vulnerability with severity rating.
    ...
    fn analyze_response(&self, response: &str, payload: &str) -> Option<WebSocketFinding> {
        ...
        // Check for command output — require uid= AND gid= together (structural UNIX output)
