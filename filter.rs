// ----------------------------------------------------------------------------
//  filter.rs — hybrid content filter with anomaly detection
// ----------------------------------------------------------------------------
//  Combines regex-based pattern matching for sensitive data (API keys, tokens,
//  passwords, JWT, credit cards) with statistical z-score anomaly detection on
//  response times and content sizes. Establishes baselines from normal traffic,
//  then flags deviations exceeding an adaptive threshold. Detects error-based
//  information disclosure (stack traces, file paths, DB driver leaks).
//  Implements an adaptive threshold that adjusts based on the false-positive
//  rate over recent detection history.
//
//  --- Developers ---------------------------------------------------------------
...
// ■ HybridContentFilter — 動的コンテンツフィルター + 異常検知
// ■ dynamic content filter with adaptive threshold + statistical anomaly + pattern matching
// ■ 3つの電脳検出フェーズ: パターンマッチ / 統計異常 / エラー情報漏洩
// ■ 3 detection phases: pattern match / statistical anomaly / error info disclosure
...
// ◆ 応答統計 / response statistics — times, lengths, status codes, error patterns
#[derive(Debug, Clone)]
...
// ◆ 電脳検出イベント / detection event — timestamp, url, type, severity, confidence, details
#[derive(Debug, Clone)]
...
// ★ 電脳検出種別 / detection type enumeration — 10 categories of security events
...
// ★ 重大度レベル / severity level — Critical / High / Medium / Low / Info
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Severity {
    Critical,
    High,
...
// ★ 抽出データ / extracted sensitive data — type, value, context, risk score
#[derive(Debug, Clone)]
...
impl ExtractedData {
    ...
    pub fn display(&self) -> String {
        let ctx_preview = self.context.get(..self.context.len().min(40))
            .unwrap_or(&self.context);
        format!(
            "[{}] {}... (context: {}...) - risk: {:.1}",
            self.data_type, &self.value, ctx_preview, self.risk_score
        )
    }
}

// ★ フィルター結果 / filter result — URL, filter decision, reasons, extracted data, anomalies
#[derive(Debug, Clone)]
...
// ■ HybridContentFilter実装 / HybridContentFilter implementation
// ■ パターン初期化 + ベースライン + 3フェーズ電脳検出 + 適応型閾値
// ■ pattern init + baseline + 3-phase detection + adaptive threshold
impl HybridContentFilter {
    pub fn new(adaptive_mode: bool) -> Result<Self> {
        ...
        // ※ 危険パターン群 / dangerous regex patterns — API keys, tokens, passwords, JWT, credit cards
    ...
    // ◆ ベースライン収集 / establish baseline from normal traffic
    // ◆ 応答時間・コンテンツサイズの平均/標準偏差を計算 / calculates mean & std dev
    ...
    // ◆ メインフィルターエントリ / main filter entry point
    // ◆ Phase 1: パターンベース電脳検出 / pattern-based static detection
    // ◆ Phase 2: 統計的異常検知 / statistical anomaly detection (z-score)
    // ◆ Phase 3: エラー情報漏洩電脳検出 / error-based information disclosure check
    ...
    pub async fn filter_content(&self, url: &str, content: &str, status_code: u16, response_time: Duration) -> FilterResult {
        ...
        // ═══ Phase 1: パターン電脳検出 / pattern-based static detection ═══
        // 機密データパターン（APIキー・トークン・パスワード等）を正規表現で電脳検出
        // detects sensitive data patterns via regex (API keys, tokens, passwords, etc.)
        ...
        for (pattern_name, regex) in patterns.iter() {
            for mat in regex.find_iter(content) {
                ...
                let ctx_short = context.get(..context.len().min(30)).unwrap_or(&context);
                result.filter_reason.push(format!("Pattern match: {} in context '{}'", pattern_name, ctx_short));
        ...
        // ═══ Phase 2: 統計的異常検知 / statistical anomaly detection ═══
        // Zスコアによる応答時間・コンテンツサイズの異常電脳検出 / z-score on time & size
        if self.adaptive_mode {
            let baseline = self.baseline_stats.read().await;
            if baseline.sample_count > 0 {
                // Check response time anomaly
                let time_ms = response_time.as_millis() as f64;
                let time_zscore = if baseline.std_dev_time > 0.0 {
                    (time_ms - baseline.baseline_avg_time) / baseline.std_dev_time
                } else {
                    0.0
                };
                ...
                let length_zscore = if baseline.std_dev_length > 0.0 {
                    (length - baseline.baseline_avg_length) / baseline.std_dev_length
                } else {
                    0.0
                };
        ...
        // ═══ Phase 3: エラー情報漏洩電脳検出 / error information disclosure ═══
        // スタックトレース・ファイルパス・DBエラー・サーバー情報を電脳検出
        // detects stack traces, file paths, DB errors, server info leaks
    ...
    // ▲ 適応型閾値調整 / adaptive threshold adjustment
    // ▲ 500秒間の電脳検出履歴に基づき閾値を調整 / adjusts based on recent 300s detection history
    // ▲ FP率が高い → 閾値上昇 / 低い → 閾値下降 / high FP → raise, low FP → lower
    ...
    // ★ エラー情報漏洩電脳検出ロジック / error-based disclosure detection logic
    // ★ Java/Python/C++ スタックトレース・ファイルパス・DBドライバ・サーバー情報
    // ★ Java/Python/C++ stack traces, file paths, DB drivers, server info
    ...
    // ※ パターンリスクスコア計算 / pattern risk score calculation
    // ※ 基本リスク × コンテキスト乗数 × 値の完全性乗数 / base × context multiplier × value multiplier
    ...
    // ➤ パターン名 → 電脳検出種別 マッピング / pattern name to DetectionType mapping
    ...
    fn extract_context(content: &str, start: usize, end: usize) -> String {
        ...
        let context_end = (end + 50).min(content_len);
        content.get(context_start..context_end)
            .map(|s| s.to_string())
            .unwrap_or_default()
    }
    ...
    fn mask_sensitive(value: &str) -> String {
        if value.len() > 8 {
            let prefix = value.get(..4).unwrap_or("");
            let suffix = value.get(value.len()-4..).unwrap_or("");
    ...
    // ◆ 電脳検出統計取得 / get detection statistics — total, by type, by severity, recent
